...
async function startCallWithFallback() {
  try {
    ...
    try {
      ...
      await Micdrop.start({
        url: 'ws://your-server.com/ws', // Try non-secure WebSocket
      ...
      showError('Unable to establish voice connection: ' + fallbackError.message)
    }
  }
}
```
...
const ERROR_MESSAGES = {
  ...
  'Connection': 'Unable to connect to voice server. Please check your internet connection and try again.',
  ...
  'InternalServer': 'Server error. Our team has been notified. Please try again in a few minutes.',
...
function simulateError(errorType) {
  const errorMap = {
    connection: new MicdropClientError('Connection', 'Simulated connection error'),
    unauthorized: new MicdropClientError('Unauthorized', 'Simulated auth error'),
