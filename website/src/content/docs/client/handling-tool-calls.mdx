...
For example, the [OpenAI Agent](../ai-integration/provided-integrations/openai.mdx#tool-call-events) emits the `ToolCall` event when a tool is called with the `emitOutput` option enabled.
