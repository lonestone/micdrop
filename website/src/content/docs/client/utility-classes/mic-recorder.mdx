...
// Start the microphone, then listen to it
await Mic.start()
...
recorder.on('Chunk', (chunk: Int16Array) => {
  // 100 ms of mono PCM16 at 16 kHz, ready to be sent to the server
...
- `Chunk`: Emitted when a new audio chunk is available, 100 ms of mono PCM16 at 16 kHz
