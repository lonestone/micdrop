...
The same methods run on a phone, where the microphone is picked the same way and the output is a route rather than a device. See [Audio output and devices](/docs/react-native/audio-output).
...
speakerDevices.forEach((device) => {
  console.log('Speaker:', device.label)
  console.log('ID:', device.id)
  console.log('Group:', device.groupId)
})
...
const newMicId = Micdrop.micDevices[1].id
...
const newSpeakerId = Micdrop.speakerDevices[1].id
...
**Example:** [MicVolume component](https://github.com/Godefroy/micdrop/blob/main/examples/advanced/client/src/components/MicVolume.tsx)
...
**Example:** [SpeakerTestButton component](https://github.com/Godefroy/micdrop/blob/main/examples/advanced/client/src/components/SpeakerTestButton.tsx)
