...
The same package covers phones, with the native ONNX runtime instead of WebAssembly:
...
A phone runs the model on its own processor, with none of the penalty a browser pays for WebAssembly, so the quantised checkpoint of eight megabytes is the right one and is what this entry point loads by default.
