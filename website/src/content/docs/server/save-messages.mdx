...
`MicdropServer` emits the same event, for every message added on either side.
This is the one to use in a call that has no agent, since nothing else keeps the
conversation:
