...
takeaways:
  - 'Pipecat and LiveKit Agents lead the field on breadth, community and production evidence. Both are the safe answer for a team that needs telephony, video or twelve client platforms.'
  - 'Server-side orchestration in Node is rarer than the marketing suggests. LiveKit Agents, Micdrop and the OpenAI Agents SDK run the voice loop natively in TypeScript, and every hosted platform ships a REST client rather than a framework.'
  ...
  - 'Hosted platforms charge a per-minute fee on top of your model bills, from a $0.05 platform fee with Vapi to $0.11 or more all-inclusive with Bland, with realistic all-in figures landing between $0.15 and $0.33. Self-hosted frameworks move that money to the infrastructure you already run.'
  - 'Micdrop publishes this ranking and sits third in it. It is also the smallest and youngest project here, with one maintainer and no telephony, and products such as Raconte and Cibli run it in production.'
  - 'Wanting the hosted-platform experience without renting it points at one project. Dograh is BSD 2-Clause, self-hosts with one Docker command, and ships the workflow builder, the call history and six carrier integrations that a platform would charge per minute for.'
---
...
Micdrop publishes this ranking, and Micdrop appears in it at number three. That is worth knowing before you read a word of the rest. Every competitor here links to its own site, the figures come from each project's own repository, licence file and pricing page, and the criteria below say plainly where Micdrop places last.
...
Every repository, licence file, pricing page and release history below was read on 13 August 2026, and the Dograh entry on 16 August 2026. Voice moves fast enough that some of it will have shifted by the time you get there.

## How the thirteen were judged

Seven criteria decided which projects made the list and what each entry has to report.
...
| **Vision Agents** | Python | WebRTC (Stream) | Python service | Yes, via Twilio | Apache 2.0 | Stream edge network |
| **Jambonz** | Node | SIP, WebSocket | Full telecom stack | Yes, its speciality | MIT | Jambonz Cloud |
| **Vocode** | Python | WebSocket | Python service | Yes, Twilio | MIT | Discontinued |
...
[Pipecat](https://pipecat.ai), from Daily, is the reference Python framework and the most starred agent framework here: 14,000 stars, 293 contributors, over 300,000 PyPI downloads a week, and a release every two or three weeks since 1.0 landed in April 2026. That 1.0 is only four months old, so the stable API is younger than the project.

Its licence is the cleanest of the two leaders. Pipecat is BSD 2-Clause, and so is its smart-turn detection model, so nothing about turn detection binds you to the framework that trained it. That single detail is a real advantage over LiveKit for anyone who cares where their models can run.

The transport catalogue is the widest of any framework on this list: ten transports, among them WebSocket and three flavours of WebRTC, plus first-party telephony serializers for Twilio, Telnyx, Plivo, Exotel, Genesys and Vonage, and SIP with call transfers. Client SDKs reach JavaScript, React, React Native, Swift, Kotlin, C++ and ESP32. Pipecat Cloud went generally available in January 2026 at $0.01 to $0.03 per agent minute, with PSTN at $0.018.
...
[Micdrop](https://micdrop.dev) is here because of one gap that the eleven others leave open. Adding a voice mode to a web application normally costs at least one of three things: a second service to deploy, a media plane to operate, or a per-minute fee stacked on your provider bills. Micdrop asks for none of the three. The browser calls `Micdrop.start({ url })`, the server hands its existing socket to `new MicdropServer(socket, options)`, and the conversation runs inside the application you already ship, on the providers you already pay.

The MIT-licensed packages carry the microphone, [voice activity detection](/docs/client/vad) and playback in the browser, the model, speech-to-text and text-to-speech on the server, and nothing else. Three behaviours that usually cost a sprint are options here. [Provider fallback](/docs/ai-integration/fallback-strategies/tts-fallback) replays buffered text on a second vendor when the first one fails, [semantic turn detection](/docs/server/semantic-turn-detection) asks the model whether the user actually finished, and a [fully European stack](/docs/ai-integration/sovereign-voice-ai) runs on Mistral, Gladia and Gradium without touching orchestration code. Ten packages have shipped continuously since 2025, past their 2.0.

One entry below avoids the same three costs. With the OpenAI Agents SDK, either the browser talks straight to OpenAI and your server drops out of the conversation, or you keep the server in the loop and write the audio capture and playback yourself, and either way the model, the transport and the bill come from one vendor.

Shipping products run the pipeline, [Raconte](https://raconte.ai) for voice interviews led by an AI and [Cibli](https://cibli.fr) for recruitment where candidates answer out loud, though Raconte is built by Micdrop's own maintainer and neither publishes a case study. The rest of the picture runs the other way. There is no telephony, no video, no mobile SDK, no managed hosting and no customer page, the community is the smallest in this ranking by a wide margin, and Pipecat and LiveKit beat it on every criterion that involves reach, adoption or the size of the team behind the code.
...
If your backend is Node and the voice feature lives inside a web application you already ship, three projects qualify: **LiveKit Agents, the OpenAI Agents SDK and Micdrop**. Separate them on two questions. Does a WebRTC media plane earn its keep for a one-to-one call between a browser and your own server, and are you willing to buy the model, the transport and the bill from one vendor? LiveKit brings a much larger community, telephony and a managed option, at the cost of an SFU and a worker process. The OpenAI Agents SDK is the least code of the three and the shortest path if you have already committed to OpenAI, since swapping any part of the pipeline means writing a transport layer. Micdrop brings a WebSocket into the server you already run with three swappable provider slots, at the cost of everything the other two do beyond that. We wrote the long version of two of those comparisons separately, [against Pipecat](/blog/alternative-to-pipecat) and [against LiveKit Agents](/blog/alternative-to-livekit-agents).
...
If what you want is a hosted platform without the hosting, **Dograh** is the only entry in this list built for that. You get the workflow builder, the call history and the carrier integrations as a Docker stack on your own machines, under a permissive licence, and you accept operating a Python service and its dependencies to have them. Weigh it against the hosted four below on operations rather than on features, since that is where the difference actually lands.

Among the hosted four, the deciding question is where your money and your data go. **Vapi** is cheapest if you bring your own keys and can live with a $0.05 floor per minute. **Bland** is simplest to forecast because everything is included, and impossible to unbundle. **Retell** has the widest contact-centre reach and no European presence. **Synthflow** is the one to shortlist when EU hosting is a requirement and a $30,000 annual floor is acceptable.
...
The short summary of this ranking is that Pipecat and LiveKit Agents are the two projects most teams should look at first, and that they ask you to adopt a Python service or a WebRTC media plane in exchange for what they cover. That trade is worth it for phone calls, video and scale. It is a lot of machinery for a voice mode inside a web app.
